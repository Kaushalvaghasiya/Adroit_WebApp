    ...
    public partial class CustomerReportController : MasterController
    {
        ...
        public JsonResult LRBookingLRRegisterReportList(int draw = 0, int start = 0, int length = 10)
        {
            ...
            try
            {
                ...
                var records = _lrBookingLRRegisterRepository.SelectList(_configurationData.DefaultConnection, CurrentUserId, CurrentBranchId, CurrentFirmId, search, start, length, sortColumn, sortDirection).ToList();
                result.data = records;
