    ...
    public partial class AdminController : MasterController
    {
        ...
        public JsonResult ProductQualityTypeList(int draw = 0, int start = 0, int length = 10, int customerId = 0)
        {
            ...
            try
            {
                ...
                var records = _productQualityTypeRepository.List(_configurationData.DefaultConnection, CurrentUserId, CurrentFirmId, search, start, length, sortColumn, sortDirection).ToList();
                result.data = records;
