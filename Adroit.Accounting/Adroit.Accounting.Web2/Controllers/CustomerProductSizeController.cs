    ...
    public partial class CustomerController : Controller
    {
        ...
        public JsonResult ProductSizeList(int draw = 0, int start = 0, int length = 10, int customerId = 0)
        {
            ...
            try
            {
                ...
                var records = _productSizeRepository.List(_configurationData.DefaultConnection, loginId, CurrentFirmId, search, start, length, sortColumn, sortDirection).ToList();
                result.data = records;
        ...
        public JsonResult SaveProductSize([FromBody] ProductSize model)
        {
            ...
            try
            {
                ...
                int id = _productSizeRepository.Save(model, userId, _configurationData.DefaultConnection);
                if (id > 0)
                {
                    result.data = true;
        ...
        public JsonResult DeleteProductSize(int id)
        {
            ...
            try
            {
                ...
                _productSizeRepository.Delete(id, userId, _configurationData.DefaultConnection);
                result.result = Constant.API_RESULT_SUCCESS;
            }
            catch (Exception ex)
            {
        ...
        public JsonResult GetProductSize(int id)
        {
            ...
            {
                int userId = LoginHandler.GetUserId(User);
                result.data = _productSizeRepository.Get(id, userId, _configurationData.DefaultConnection);
                result.result = Constant.API_RESULT_SUCCESS;
            }
            catch (Exception ex)
            {
