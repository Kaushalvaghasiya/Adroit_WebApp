    ...
    public partial class CustomerReportController : MasterController
    {
        ...
        public JsonResult LRBookingStockRegisterReportList(int draw = 0, int start = 0, int length = 10)
        {
            ...
            try
            {
                ...
                var records = _lrBookingStockRegisterRepository.SelectList(_configurationData.DefaultConnection, CurrentUserId, CurrentBranchId, CurrentFirmId, search, start, length, sortColumn, sortDirection).ToList();
                result.data = records;
