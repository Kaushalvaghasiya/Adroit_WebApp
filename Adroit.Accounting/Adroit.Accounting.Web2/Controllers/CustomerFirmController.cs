...
namespace Adroit.Accounting.Web.Controllers
{
    public partial class CustomerController : MasterController
    {
        ...
        public JsonResult SaveCustomerFirm([FromBody] CustomerFirm model)
        {
            ...
            try
            {
                int id = _customerFirmRepository.Save(model, CurrentUserId, CurrentFirmId, _configurationData.DefaultConnection);
                if (id > 0)
                {
                    result.data = true;
        ...
        public JsonResult DeleteCustomerFirm(int id)
        {
            ...
            try
            {
                _customerFirmRepository.Delete(id, CurrentUserId, CurrentFirmId, _configurationData.DefaultConnection);
                result.result = Constant.API_RESULT_SUCCESS;
            }
            catch (Exception ex)
            {
