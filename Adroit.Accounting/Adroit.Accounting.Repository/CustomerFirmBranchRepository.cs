...
namespace Adroit.Accounting.Repository
{
    public class CustomerFirmBranchRepository : ICustomerFirmBranch
    {
        ...
        public List<DropdownViewModel> SelectListByCustomerId(int firmId, string connectionString)
        {
            var parameters = new DynamicParameters();
            parameters.Add("@FirmId", firmId);
            return QueryHelper.GetList<DropdownViewModel>("sp_CustomerFirmBranchListByCustomerId_Select", connectionString, parameters);
        }
        public List<DropdownViewModel> SelectListWithFirmByFirmId(int firmId, string connectionString)
        {
            var parameters = new DynamicParameters();
            parameters.Add("@FirmId", firmId);
            return QueryHelper.GetList<DropdownViewModel>("sp_CustomerFirmBranchListWithFirmName_Select", connectionString, parameters);
        }
        public List<DropdownViewModel> SelectListForPendingChalan(int loginId, string connectionString)
        {
            var parameters = new DynamicParameters();
            parameters.Add("@LoginId", loginId);
            return QueryHelper.GetList<DropdownViewModel>("sp_CustomerFirmBranchListForPendingChalan_Select", connectionString, parameters);
        }
        public List<DropdownViewModel> SelectListWithBranchToBranchMapping(int loginId, string connectionString)
        {
            var parameters = new DynamicParameters();
            parameters.Add("@LoginId", loginId);
            return QueryHelper.GetList<DropdownViewModel>("sp_CustomerFirmBranchListByLoginId_Select", connectionString, parameters);
