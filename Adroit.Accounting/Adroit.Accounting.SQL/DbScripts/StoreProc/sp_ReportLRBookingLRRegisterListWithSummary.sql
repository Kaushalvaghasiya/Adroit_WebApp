...
Begin
	...
	WITH CTE AS (
		...
		(ORDER BY
			CASE WHEN @SortColumn = 0 AND @SortOrder ='ASC' THEN CASE WHEN @SelectedView = 'DateWise' THEN CAST(LBR.LRDate AS VARCHAR(100)) WHEN @SelectedView = 'PartyWise' THEN CA1.Name END END ASC,  
			CASE WHEN @SortColumn = 0 AND @SortOrder ='DESC' THEN CASE WHEN @SelectedView = 'DateWise' THEN CAST(LBR.LRDate AS VARCHAR(100)) WHEN @SelectedView = 'PartyWise' THEN CA1.Name END END DESC,
		...
		(
			...
			AND
			(
				(@LRFrom = '0' OR LBR.LRNumber >= @LRFrom)
			)
			AND
			(
				(@LRTo = '0' OR LBR.LRNumber <= @LRTo)
			)
			AND
			(
			...
			AND
			(
				(@PvtMarkIds = '0' OR LBR.PrivateMarka IN (SELECT value FROM STRING_SPLIT(@PvtMarkIds, ',')))
			)
			AND
			(
        ...
        GROUP BY CASE WHEN @SelectedView = 'DateWise' THEN CAST(LBR.LRDate AS VARCHAR(100)) WHEN @SelectedView = 'PartyWise' THEN CA1.Name END
    )

	SELECT * FROM CTE
    WHERE
        ((@PageSize = -1 AND 1 = 1) OR (RowNum > @PageStart AND RowNum < (@PageStart + (@PageSize + 1))))
        AND (Coalesce(@Search, '') = '' OR GroupingColumn LIKE '%' + @Search + '%'
